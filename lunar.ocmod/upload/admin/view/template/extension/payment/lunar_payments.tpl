...
?>
    ...
    <script type="text/javascript"><!--
        $('#button-filter').on('click', function () {
            ...
            location = 'index.php?route=extension/payment/lunar/payments&<?php echo $url_token_param; ?>' + url;
        });
        //--></script>
    ...
    <div class="modal fade" id="module-payments" role="dialog">
    ...
    <script type="text/javascript"><!--
        $('body').on('click', '.newtransaction a', function (e) {
            e.preventDefault();
            $('#module-payments #plt-history').html('');
            $('#module-payments .runtransaction').show();
            $('#module-payments #plt-history').hide();
            $('#module-payments #plt-transaction').show();
            $('#module-payments #plt-result').html('');
            $('#module-payments .modal-title').text('<?php echo $popup_title_transaction; ?>');
            $('#module-payments #plt-transaction > p').text('<?php echo $popup_description; ?>');
            $('#module-payments #plt-ref').val($(this).data('ref'));
            $('#module-payments #plt-type').val($(this).data('type'));
            $('#module-payments #plt-amount').val($(this).data('amount'));
            $('#module-payments #plt-c').text($(this).data('currency'));
            if ($(this).data('type') == 'Refund') {
                $('#module-payments #plt-amount').attr('readonly', false);
            } else {
                $('#module-payments #plt-amount').attr('readonly', true);
            }
            $('#module-payments').modal('show');
        });

        $('body').on('click', '.runtransaction', function (e) {
            e.preventDefault();
            $.ajax({
                url: 'index.php?route=extension/payment/lunar/transaction&<?php echo $url_token_param; ?>',
                dataType: 'json',
                type: 'POST',
                data: 'ref=' + $('#module-payments #plt-ref').val() + '&type=' + $('#module-payments #plt-type').val() + '&amount=' + $('#module-payments #plt-amount').val(),
                success: function (data, textStatus, jQxhr) {
                    if (data.error) {
                        $('#module-payments #plt-result').html('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + data.error + '</div>');
                    } else {
                        $('#module-payments #plt-result').html('<div class="alert alert-success"><i class="fa fa-exclamation-circle"></i> ' + data.success + '</div>');
                        $('#module-payments .runtransaction').hide();
                ...
                error: function (jqXhr, textStatus, errorThrown) {
                    ...
                    $('#module-payments #plt-result').html('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + errorThrown + '</div>');
                }
            });
        });
        ...
        $('body').on('click', '.gethistory', function (e) {
            ...
            $('#module-payments .runtransaction').hide();
            $('#module-payments #plt-result').html('');
            $('#module-payments #plt-transaction').hide();
            $('#module-payments #plt-history').show();
            $('#module-payments #plt-ref').val('');
            $('#module-payments #plt-type').val('');
            $('#module-payments #plt-amount').val('');
            $('#module-payments #plt-c').text('');
            $('#module-payments').modal('show');
            $.ajax({
                url: $(this).attr('href'),
                dataType: 'html',
                type: 'GET',
                success: function (data, textStatus, jQxhr) {
                    $('#module-payments #plt-history').html(data);
                },
                error: function (jqXhr, textStatus, errorThrown) {
                    console.log(errorThrown);
                }
